...
import kotlinx.android.synthetic.main.fragment_gank_girl.rv_gank_girl
    ...
    mGankGirlAdapter.apply {
      setOnItemClickListener(object : OnItemClickListener<GankEntity> {
        ...
        override fun onItemClick(p0: ViewGroup?,
          ...
          mGankGirlAdapter.data.forEach {
            picList.add(it.url.toString())
            ids.add(it._id.toString())
          }
          intent.putExtra(BigMeiziActivity.EXTRA_PIC_LIST, picList)
          intent.putExtra(BigMeiziActivity.EXTRA_ID, ids)
          intent.putExtra(BigMeiziActivity.EXTRA_POSITION, position)
